...
def get_signal_pnl(file, product, signal_name, thre_mat, reverse=1,  buy_tranct=1.5e-4, sell_tranct=11.5e-4,
    ...
    for thre in thre_mat.iterrows():
        ...
        change_base[change_buy] = data["next.ask"][change_buy]*(1+buy_tranct)*data["adjust"][change_buy] ## buy price, use next ask, tranct cost use notional*ratio
        bad_buy = change_base<1e-8 
        change_base[bad_buy] = data["wpr"][bad_buy]*(1+buy_tranct)
        change_base[change_sell] = data["next.bid"][change_sell]*(1-sell_tranct)*data["adjust"][change_sell] ## sell price use next bid
        bad_sell = change_base<1e-8
        change_base[bad_sell] = data["wpr"][bad_sell]*(1-sell_tranct)## sell price use next bid
...
def par_get_all_signal(signal_name, file_list, product, period, SAVE_PATH="e:/intern"):
    ...
    all_signal = np.array([])
    for file in file_list:
        signal = load(SAVE_PATH+"/tmp pkl/"+product+"/"+signal_name+"/"+file) ## signal
        chosen = (np.arange(len(signal))+1) % period==0
        all_signal = np.concatenate((all_signal, signal[chosen]), axis=0)
